...
def compare_faces(reference_face, new_face):
    ...
    return np.mean(difference) < 35  # Adjusted threshold for better matching 
    """
...
def main():
    ...
    reference_label = get_label_from_filename(reference_image_path)
            ...
            if confidence > 0.5:  # Confidence threshold
                ...
                try:
                    if compare_faces(reference_face, face):
                        labels.append(reference_label)  # Use filename-based label
                    else:
